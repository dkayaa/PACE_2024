...
class PaceGraph:
    ...
    def left(self):
        return range(0, self.a)
    
    def right(self):
        return range(self.a, self.a + self.b)

def empty_bipartite(a, b):
    graph = PaceGraph(a, b)
    graph.G.add_nodes_from(range(0,a+b))
    
    return graph

def read_graph(filename: str) -> PaceGraph:
    graph = None
    with open(filename, "r") as f:
        ...
        for line in f:
            if line[0] == "p":
                ...
                graph = empty_bipartite(a, b)
            elif line[0] == "c":
                pass
            elif graph:
                edges.append(list(map(int, line.split(" "))))
            else:
                print("ERROR: Encountered edge before p-line.")
                
        graph.G.add_edges_from(edges)
